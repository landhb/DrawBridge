...
{ 
    ...
    match (src_ip, dst_ip) {
        ...
        (IpAddr::V6(src_ip6), IpAddr::V6(dst_ip6)) => {
            let checksum = pnet::packet::udp::ipv6_checksum(&udp.to_immutable(), &src_ip6, &dst_ip6);
            udp.set_checksum(checksum);
        }
...
{ 
    ...
    match (src_ip, dst_ip) {
        ...
        (IpAddr::V6(src_ip6), IpAddr::V6(dst_ip6)) => {
            let checksum = pnet::packet::tcp::ipv6_checksum(&tcp.to_immutable(), &src_ip6, &dst_ip6);
            tcp.set_checksum(checksum);
        }
        _ => {bail!("[-] Unknown IP Address type")}
    }
